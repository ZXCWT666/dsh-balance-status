...
function aggregateWindows(folds, fromMs, toMs) {
	const out = { input: 0, output: 0, cacheRead: 0, reasoning: 0, calls: 0, models: {} };
	for (const days of folds) {
		...
		for (const [day, bucket] of days) {
			...
			out.cacheRead += bucket.cacheRead;
			out.reasoning += bucket.reasoning;
			out.calls += bucket.calls;
			for (const [model, mb] of bucket.models) {
				const held = out.models[model];
				if (held === undefined) out.models[model] = { input: mb.input, output: mb.output, cacheRead: mb.cacheRead, reasoning: mb.reasoning, calls: mb.calls };
				else {
					held.input += mb.input;
					held.output += mb.output;
					held.cacheRead += mb.cacheRead;
					held.reasoning += mb.reasoning;
					held.calls += mb.calls;
				}
			}
		}
...
// ── display targets (configurable) ─────────────────────────────────────────

/** Persisted settings namespace of this plugin (user layer in $DSH_HOME/settings.yaml). */
const TARGETS_NS = "balance-status";
/** Environment overrides for the display targets (highest precedence). */
const ENV_BALANCE_TARGET = "BALANCE_STATUS_BALANCE_TARGET";
const ENV_DAILY_TOKENS = "BALANCE_STATUS_DAILY_TOKENS";

/**
 * Resolve the energy-bar display targets: settings (user layer) → environment
 * variables → compile defaults. Settings edits apply on the next snapshot
 * without a restart; environment overrides win by design.
 * @param ctx - the plugin context.
 * @returns { balance, dailyTokens }.
 */
function resolveTargets(ctx) {
	...
	if (settings !== undefined) {
		try {
			const section = settings.get(TARGETS_NS);
			if (section !== null && typeof section === "object") {
				if (Number.isFinite(section.balance) && section.balance > 0) resolved.balance = section.balance;
				if (Number.isFinite(section.dailyTokens) && section.dailyTokens > 0) resolved.dailyTokens = section.dailyTokens;
			}
		} catch {
			/* namespace not registered yet: defaults. */
		}
	}
	const envBalance = Number(process.env[ENV_BALANCE_TARGET]);
	if (Number.isFinite(envBalance) && envBalance > 0) resolved.balance = envBalance;
	const envDaily = Number(process.env[ENV_DAILY_TOKENS]);
	if (Number.isFinite(envDaily) && envDaily > 0) resolved.dailyTokens = envDaily;
	return resolved;
}
...
/**
 * Mount the settings namespace (user-configurable display targets), the
 * route and the status service.
...
function apply(ctx) {
	...
	if (settings !== undefined) {
		try {
			ctx.effect(() => settings.register(TARGETS_NS, z.object({
				balance: z.number().min(1).default(BALANCE_DISPLAY_TARGET),
				dailyTokens: z.number().min(1).default(DAILY_TOKEN_TARGET)
			})), "balance-status: settings namespace");
		} catch {
			/* namespace registration is best-effort; env/defaults still apply. */
		}
	}
